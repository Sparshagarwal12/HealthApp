...
public class community_reg extends AppCompatActivity implements AdapterView.OnItemSelectedListener {

    EditText org_name;
    EditText org_phone;
    EditText org_aadhar;
    EditText ambulance_quantity;
    EditText org_add;
    EditText org_password;
    EditText opt_name;
    TextView lat1, lont;
    String stitem,diitem,depart,serve;
    int st,dt,sp;
    Location currentLocation;
    Button submit,locate,depart_type;
    FusedLocationProviderClient fusedLocationProviderClient;
    private static final int REQUEST_CODE = 101;
    Spinner state,district,service;
    ...
    ArrayList<String> ambulance_serve = new ArrayList<>(Arrays.asList(
            "सरकारी एम्बुलेंस सेवा",

            "फायर या पुलिस लिंक्ड सर्विस",

            "स्वयंसेवी एम्बुलेंस सेवा",

            "निजी एम्बुलेंस सेवा",

            "जननी एक्सप्रेस योजना (सरकारी- निजी)"
    ));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        org_aadhar =findViewById(R.id.organisation_aadhar);
        service = findViewById(R.id.ambulance_service);
        org_name = findViewById(R.id.organisation_name);
        org_password = findViewById(R.id.organisation_password);
        ambulance_quantity = findViewById(R.id.ambulance_quantity);
        lont = findViewById(R.id.lon);
        lat1 = findViewById(R.id.lat);
        district = findViewById(R.id.organisation_dist);
        org_phone =findViewById(R.id.organisation_phone);
        org_add = findViewById(R.id.organisation_address);
        state = findViewById(R.id.organisation_state);
        opt_name =findViewById(R.id.organisation_operator_name);
        submit = findViewById(R.id.organisation_submit);
        locate = findViewById(R.id.get_loc);


        ArrayAdapter<String> dataAdapter_serve = new ArrayAdapter<>(community_reg.this, android.R.layout.simple_spinner_item, ambulance_serve);
        service.setAdapter(dataAdapter_serve);

        service.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {

                serve = parent.getItemAtPosition(position).toString();
                sp = position;
            }
        ...
        ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(community_reg.this, android.R.layout.simple_spinner_item, states);
        ...
        state.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(community_reg.this, android.R.layout.simple_spinner_item, districts.get(position));
        ...
        public void onClick(View v) {
            fusedLocationProviderClient = LocationServices.getFusedLocationProviderClient(community_reg.this);
            fetchLastLocation();
        }
    });
    ...
    private void community_register(){

        String operator_name = opt_name.getText().toString().trim();
        String organisation_phone = org_phone.getText().toString().trim();
        String organisation_aadhar = org_aadhar.getText().toString().trim();
        String amb_qty = ambulance_quantity.getText().toString().trim();
        String organisation_address = org_add.getText().toString().trim();
        String organisation_name = org_name.getText().toString().trim();
        String organisation_password = org_password.getText().toString().trim();
        String latitude = lat1.getText().toString().trim();
        String org_service = service.getSelectedItem().toString();
        String org_state = state.getSelectedItem().toString();
        String org_district = district.getSelectedItem().toString();
        String longitude = lont.getText().toString().trim();

        if(organisation_aadhar.isEmpty()){
            org_aadhar.setError("Enter Operator Name");
            org_aadhar.requestFocus();
        }
        if (organisation_aadhar.length() != 12) {
            org_aadhar.setError("Enter Valid Aadhar Number");
            org_aadhar.requestFocus();
            return;
        }
        if(organisation_name.isEmpty()){
            org_name.setError("Enter Organisation Name");
            org_name.requestFocus();
        }
        if(organisation_password.isEmpty()){
            org_password.setError("This Field is Required");
        }
        if(amb_qty.isEmpty()){
            ambulance_quantity.setError("This Field is Required");
            ambulance_quantity.requestFocus();
        }
        if(organisation_phone.isEmpty()){
            org_phone.setError("Enter Operator Name");
            org_phone.requestFocus();
        }
        if (organisation_phone.length() != 10) {
            org_phone.setError("This Fiels is Required");
            org_phone.requestFocus();
            return;
        }
        if(organisation_address.isEmpty()){
            org_add.setError("This Field is Required");
            org_add.requestFocus();
        }

        if(operator_name.isEmpty()){
            opt_name.setError("Enter Operator Name");
        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                try {
                    ...
                    Toast.makeText(community_reg.this,"User Created Successfully",Toast.LENGTH_LONG).show();
                    Intent intent = new Intent(community_reg.this, ambulance_login.class);
