...
public class lab_operator_reg extends AppCompatActivity {

    EditText email1, adhar, opt_name, hos_name, opt_mobile, hos_tehsil, password,hos_village ,hospital_region,verify,get_id,staff_type;
    TextView lat1, lont;
    String stitem,diitem,opt;
    int st,dt,ot;
    LinearLayout after_layout,before_layout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lab_operator_reg);

        locate = findViewById(R.id.get_loc_lab);
        verify_button = findViewById(R.id.lab_verification);

        after_layout = findViewById(R.id.after_verify_test);
        before_layout = findViewById(R.id.before_verify_test);

        state = findViewById(R.id.hospital_lab_state);
        district = findViewById(R.id.hospital_lab_dist);
        ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(lab_operator_reg.this, android.R.layout.simple_spinner_item, states);
        ...
        state.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(lab_operator_reg.this, android.R.layout.simple_spinner_item, districts.get(position));
...
//                dt = position;
        ...
        locate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                fusedLocationProviderClient = LocationServices.getFusedLocationProviderClient(lab_operator_reg.this);
                fetchLastLocation();
            }
        });

        verify = findViewById(R.id.lab_verify);
        email1 = findViewById(R.id.lab_email);
        hos_name = findViewById(R.id.lab_name);
        opt_name = findViewById(R.id.lab_operator_Name);
        adhar = findViewById(R.id.lab_operator_aadhar);
        opt_mobile = findViewById(R.id.lab_mobile);
        password = findViewById(R.id.lab_password);
        hos_tehsil = findViewById(R.id.hospital_lab_tehsil);
        hos_village = findViewById(R.id.hospital_lab_village);
        hospital_region = findViewById(R.id.hospital_lab_region);
        submit = findViewById(R.id.button_submit_lab);
        staff_type = findViewById(R.id.staff_type);
                ...
                call.enqueue(new Callback<wardVerify>() {
                    ...
                    public void onResponse(Call<wardVerify> call, Response<wardVerify> response) {
                        if(response.code()==200){
                            Toast.makeText(lab_operator_reg.this,"Verified",Toast.LENGTH_LONG).show();
                            staff_id = response.body().getStaffId();
                           // sname.setText(response.body().getFirst_name());
                            before_layout.setVisibility(View.VISIBLE);
//                            after_layout.setVisibility(View.GONE);
                        }else {
                            Toast.makeText(lab_operator_reg.this,"Couldnot verify with this key",Toast.LENGTH_LONG).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<wardVerify> call, Throwable t) {
                        Toast.makeText(lab_operator_reg.this,t.getMessage(),Toast.LENGTH_LONG).show();
                    }
                });
            }
        });
    ...
    private void test_register(){
        ...
        String lab_operator_aadhar =adhar.getText().toString().trim();
        String lab_password = password.getText().toString().trim();
        String lab_email = email1.getText().toString().trim();
        String lab_name = hos_name.getText().toString().trim();
        String lab_tehsil = hos_tehsil.getText().toString().trim();
        String lab_latitude = lat1.getText().toString().trim();
        String lab_longitude = lont.getText().toString().trim();
        String lab_mobile= opt_mobile.getText().toString().trim();
        ...
        String s_type = staff_type.getText().toString().trim();
...
//                Toast.makeText(lab_operator_reg.this,"User Created Successfully",Toast.LENGTH_LONG).show();
                try {
