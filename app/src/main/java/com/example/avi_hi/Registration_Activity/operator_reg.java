...
public class operator_reg extends AppCompatActivity implements View.OnClickListener{

    EditText d_email,d_pass,d_name,d_phn,d_aadhar,d_licence,d_exp,d_age;
    Button driver_detail_submit;
    String stitem;
    int st,dt,dpt;
    com.example.avi_hi.operator operator;
...
//    ));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_operator_reg);

        d_email = findViewById(R.id.driver_email);
        d_pass = findViewById(R.id.driver_password);
        d_name = findViewById(R.id.driver_name);
        d_phn = findViewById(R.id.driver_phone);
        d_aadhar = findViewById(R.id.driver_aadhar);
        d_licence = findViewById(R.id.driver_licence);
        d_exp = findViewById(R.id.driver_experience);
        d_age = findViewById(R.id.driver_age);
        driver_detail_submit = findViewById(R.id.driver_submit);

        mAuth = FirebaseAuth.getInstance();
        final FirebaseDatabase database = FirebaseDatabase.getInstance();
        final DatabaseReference reff = database.getReference("Driver Registration");

        state = findViewById(R.id.driver_state);
        district = findViewById(R.id.driver_district);

        ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(operator_reg.this, android.R.layout.simple_spinner_item, states);
        ...
        state.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(operator_reg.this, android.R.layout.simple_spinner_item, districts.get(position));
        ...
        driver_detail_submit.setOnClickListener(this);

    }

    //Registration Detail of Driver

    private void driver_register(){
        String driver_mail = d_email.getText().toString().trim();
        String driver_pass = d_pass.getText().toString().trim();
        String driver_name = d_name.getText().toString().trim();
        String driver_phone = d_phn.getText().toString().trim();
        String driver_aadhar = d_aadhar.getText().toString().trim();
        String driver_licence = d_licence.getText().toString().trim();
        String driver_experience = d_exp.getText().toString().trim();
        String driver_age = d_age.getText().toString().trim();
        String driver_state = state.getSelectedItem().toString();
        String driver_district = district.getSelectedItem().toString();


        if (driver_mail.isEmpty()) {
            d_email.setError("Enter Driver email id");
            d_email.requestFocus();
        }

        if(driver_pass.isEmpty()){
            d_pass.setError("Enter the Password");
            d_pass.requestFocus();
        }
        if(driver_pass.length()<6){
            d_pass.setError("Password is too Short");
            d_pass.requestFocus();
        }
        if (driver_name.isEmpty()) {
            d_name.setError("Enter Driver Name");
            d_name.requestFocus();
        }
        if (driver_phone.isEmpty()) {
            d_phn.setError("Enter Driver Phone Number");
            d_phn.requestFocus();
        }
        if (driver_aadhar.isEmpty()) {
            d_aadhar.setError("Enter Driver Aadhar Number");
            d_aadhar.requestFocus();
        }
        if (driver_aadhar.length()!=12) {
            d_aadhar.setError("Enter 12 Digit Aadhar Number");
            d_aadhar.requestFocus();
        }
        if (driver_licence.isEmpty()) {
            d_licence.setError("Enter Driver Licence");
            d_licence.requestFocus();
        }
        if (driver_experience.isEmpty()) {
            d_exp.setError("Enter Driver Experience");
            d_exp.requestFocus();
        }
        if (driver_age.isEmpty()) {
            d_age.setError("Enter Driver Age");
            d_age.requestFocus();
        }
...
//                Toast.makeText(operator_reg.this,"User Created Successfully",Toast.LENGTH_LONG).show();
//                try {
//                    String s = response.body().string();
//                    Toast.makeText(operator_reg.this,"User Created Successfully"+s,Toast.LENGTH_LONG).show();
//                } catch (IOException e) {
//                    e.printStackTrace();
//                }
//            }
