...
public class test_register extends AppCompatActivity {
    ...
    EditText email1, adhar, opt_name, hos_name, opt_mobile, hos_tehsil, password, hos_village, hospital_region, verify,test_opt_type;

    TextView test_id;
    TextView lat1, lont;
    String stitem, diitem, opt;
    int st, dt, ot;
    Location currentLocation;
    Button submit, locate, verify_button;
    com.example.avi_hi.hospital hospital;
    int flag = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        locate = findViewById(R.id.get_loc_hospital);
        verify = findViewById(R.id.test_verify);
        verify_button = findViewById(R.id.test_verify_button);

        state = findViewById(R.id.hospital_test_state);
        district = findViewById(R.id.hospital_test_dist);
        ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(test_register.this, android.R.layout.simple_spinner_item, states);
        ...
        state.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(test_register.this, android.R.layout.simple_spinner_item, districts.get(position));
        ...
        locate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                fusedLocationProviderClient = LocationServices.getFusedLocationProviderClient(test_register.this);
                fetchLastLocation();
            }
        });

        verify = findViewById(R.id.test_verify);
        email1 = findViewById(R.id.test_email);
        hos_name = findViewById(R.id.test_name);
        opt_name = findViewById(R.id.test_operator_Name);
        adhar = findViewById(R.id.test_operator_aadhar);
        opt_mobile = findViewById(R.id.test_operator_mobile);
        password = findViewById(R.id.test_password);
        hos_tehsil = findViewById(R.id.hospital_test_tehsil);
        hospital_region = findViewById(R.id.hospital_test_region);
        hos_village = findViewById(R.id.hospital_test_village);
        submit = findViewById(R.id.button_submit_test);
       // test_id = findViewById(R.id.hospital_test_id);
        test_opt_type = findViewById(R.id.operator_type);
...
//            @Override
//            public void onClick(View v) {
//                String var = verify.getText().toString().trim();
//                if (var.equals("1")) {
//                    LinearLayout mLayout = findViewById(R.id.after_verify);
//                    mLayout.setVisibility(View.VISIBLE);
//                    Toast.makeText(getApplicationContext(), "Key Matched", Toast.LENGTH_SHORT).show();
//                    flag = 1;
//                } else {
//
//                    Toast.makeText(getApplicationContext(), "Key Not Matched", Toast.LENGTH_SHORT).show();
//                }
//            }
//        });
        ...
        verify_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String key1 = verify.getText().toString().trim();
                if (key1.isEmpty()) {
                    verify.setError("Key is Required");
                    verify.requestFocus();
                    return;
                }
                Call<wardVerify> call= RetrofitClient.getInstance()
                        .getInterface()
                        .verifyOperator(key1);
                call.enqueue(new Callback<wardVerify>() {
                    @Override
                    public void onResponse(Call<wardVerify> call, Response<wardVerify> response) {
                        if(response.code()==200){
                            Toast.makeText(test_register.this,"Verified",Toast.LENGTH_LONG).show();
                            Staff_id = response.body().getStaffId();
//                            test_id.setText(Staff_id);
                            LinearLayout mLayout = findViewById(R.id.after_verify_test);
                            mLayout.setVisibility(View.VISIBLE);
                        }else {
                            Toast.makeText(test_register.this,"Couldnot verify with this key",Toast.LENGTH_LONG).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<wardVerify> call, Throwable t) {
                        Toast.makeText(test_register.this,t.getMessage(),Toast.LENGTH_LONG).show();
                    }
                });
            }
        });
    ...
    private void ward_register(){

        String test_operator_aadhar =adhar.getText().toString().trim();
        String test_password = password.getText().toString().trim();
        String test_email = email1.getText().toString().trim();
        String test_name = hos_name.getText().toString().trim();
        String test_tehsil = hos_tehsil.getText().toString().trim();
        String test_latitude = lat1.getText().toString().trim();
        String test_longitude = lont.getText().toString().trim();
        String test_mobile= opt_mobile.getText().toString().trim();
        String hos_region = hospital_region.getText().toString().trim();
        String hospital_state = state.getSelectedItem().toString();
        String hospital_district = district.getSelectedItem().toString();
        String hospital_operator_name = opt_name.getText().toString().trim();
        String hospital_village= hos_village.getText().toString().trim();
        String staff_type = test_opt_type.getText().toString().trim();
//        String hospital_id = test_id.getText().toString().trim();
        Call<ResponseBody> call= RetrofitClient.getInstance()
                .getInterface()
                .createLab(
                        test_operator_aadhar,
                        staff_type,
                        test_password,
                        test_email,
                        hospital_operator_name,
                        test_tehsil,
                        Staff_id,
                        test_latitude,
                        test_longitude,
...
//                Toast.makeText(test_register.this,"User Created Successfully",Toast.LENGTH_LONG).show();
                try {
                    Intent intent = new Intent(test_register.this, hospital_login.class);
